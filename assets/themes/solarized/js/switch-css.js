...
// you do not remove this notice or the URL below. 
  ...
  for (var i = 0; i < links.length ; i++ ) {
        var link=links[i];
    if (link.rel && link.title=='theme' 
        && link.rel.indexOf( "stylesheet")>-1 //found
        && link.href.indexOf(theme+'.css')==-1) {//not found
        link.href=link.href.replace(/[^\/]+\.css$/,theme+'.css');
        set_cookie( style_cookie_name, theme, style_cookie_duration );
        break;
    }
  }
}
